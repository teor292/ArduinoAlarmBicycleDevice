...
/*
This library is modified to use time() func from time_utils.h and not millis()
Because millis() don't tick when controller sleep, and time() - tick
*/
...
public:
   ...
   uint32_t age() const    { return valid ? time_to_ms_check(time() - lastCommitTime) : (uint32_t)ULONG_MAX; }
